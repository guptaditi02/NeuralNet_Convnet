...
def UDA_pytorch_classifier_fit(model, optimizer, loss,
    ...
    proper_train_accuracies = np.zeros(num_epochs)
    ...
    for epoch_idx in tqdm(range(num_epochs)):
        ...
        print('  Proper training accuracy: %.4f' % proper_train_accuracy, flush=True)
        proper_train_accuracies[epoch_idx] = proper_train_accuracy
    ...
    return proper_train_accuracies, val_accuracies
...
def UDA_plot_train_val_accuracy_vs_epoch(proper_train_accuracies, val_accuracies):
    """
    Helper function for plotting (proper) training and validation accuracies
    across epochs; `proper_train_accuracies` and `val_accuracies` should be the same
    length, which should equal the number of epochs.
    """
    ax = plt.figure().gca()
    num_epochs = len(proper_train_accuracies)
    plt.plot(np.arange(1, num_epochs + 1), proper_train_accuracies, '-o',
